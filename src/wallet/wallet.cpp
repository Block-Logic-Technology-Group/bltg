        ...
        int blockHeight, bool fPrecompute)
{
    ...
    /* Disable zBLTG Staking
    //zBLTG
    if ((GetBoolArg("-zbltgstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        std::set<CMintMeta> setMints = zbltgTracker->ListMints(true, true, fUpdate);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 BLTG collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 BLTG collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 BLTG.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 BLTG.");
                    } else {
        ...
        bool isPublicSpend)
{
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zBLTG public spend transaction
...
std::string CWallet::GetUniqueWalletBackupName(bool fzbltgAuto) const
{
...
void CWallet::ZBltgBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozbltgbackup-%d.dat", i + 1);
    ...
    if(!GetArg("-zbltgbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zbltgbackuppath", ""));
...
{
    // We don't even need to worry about this code.. no zBLTG.
    /*
...
{
    zwalletMain = zwallet;
    zbltgTracker = std::unique_ptr<CzBLTGTracker>(new CzBLTGTracker(strWalletFile));
}
...
void CWallet::setZBltgAutoBackups(bool fEnabled)
{
