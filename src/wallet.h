...
enum AvailableCoinsType {
    ALL_COINS = 1,
    ONLY_DENOMINATED = 2,
    //Todo Make these constants more neutral towards collateral changes
    ONLY_NOT12000IFMN = 3,
    ONLY_NONDENOMINATED_NOT12000IFMN = 4, // ONLY_NONDENOMINATED and not 12000 BLTG at the same time
...
public:
    ...
    int Priority() const
    {
        BOOST_FOREACH (CAmount d, obfuScationDenominations)
            if (tx->vout[i].nValue == d) return 12000;
        if (tx->vout[i].nValue < 1 * COIN) return 24000;
