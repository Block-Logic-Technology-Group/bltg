...
#include "qt/bltg/forms/ui_privacywidget.h"
...
PrivacyWidget::PrivacyWidget(BLTGGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zBLTG anonymizes your BLTG by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zBLTG or convert back to BLTG"));
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zBLTG");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zBLTG");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zBLTG");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zBLTG");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zBLTG");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zBLTG");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zBLTG");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zBLTG");
    ...
    // Only Convert to BLTG enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzBLTG->setTitleClassAndText("btn-title-grey", "Total 0 zBLTG");
    ui->btnTotalzBLTG->setSubTitleClassAndText("text-subtitle", "Show denominations of zBLTG owned.");
    ui->btnTotalzBLTG->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BLTG outputs to mint into zBLTG.");
    ...
    connect(ui->btnTotalzBLTG, SIGNAL(clicked()), this, SLOT(onTotalZbltgClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zBLTG");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of BLTG to mint into zBLTG"));
    }else{
        btnText = tr("Convert back to BLTG");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zBLTG to convert back into BLTG"));
    }
...
void PrivacyWidget::onTotalZbltgClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzBLTG->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzBLTG->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zBLTG is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zBLTG, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zBLTG").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zBLTG minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZbltg(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zBLTG converted back to BLTG"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzBLTG->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
