                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zBLTG ought to be enough for anybody." - Bill Gates, 2017
    ui->zBLTGpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBLTGSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBLTGpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzBLTG_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBLTG is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zBLTG, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBLTG...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBLTG in ") +
...
void PrivacyDialog::on_pushButtonSpendzBLTG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBLTG is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zBLTG, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zBLTG
        sendzBLTG();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBLTG
    sendzBLTG();
}

void PrivacyDialog::on_pushButtonZBltgControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
...
void PrivacyDialog::setZBltgControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBltgSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBLTG()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Bltg Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBLTGpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBLTG is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BLTG </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBLTG</b>";
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zBLTG was spent successfully update the addressbook with the label
    ...
    // Clear zbltg selector in case it was used
    ZBltgControlDialog::setSelectedMints.clear();
    ui->labelzBltgSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBLTG Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Bltg, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zBLTG Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zbltgTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBLTG </b>";
...
//    updateAutomintStatus();
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zBLTG </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzBLTGSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzBLTG->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zBLTG
        //ui->pushButtonMintzBLTG->setEnabled(false);
        //ui->pushButtonMintzBLTG->setToolTip(tr("zBLTG is currently disabled due to maintenance."));

        // Spend zBLTG
        ui->pushButtonSpendzBLTG->setEnabled(false);
        ui->pushButtonSpendzBLTG->setToolTip(tr("zBLTG is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zBLTG
        //ui->pushButtonMintzBLTG->setEnabled(true);
        //ui->pushButtonMintzBLTG->setToolTip(tr("PrivacyDialog", "Enter an amount of BLTG to convert to zBLTG", 0));

        // Spend zBLTG
        ui->pushButtonSpendzBLTG->setEnabled(true);
        ui->pushButtonSpendzBLTG->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
