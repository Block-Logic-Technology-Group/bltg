...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"bltgaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"bltgaddress\"  (string, required) The bltg address to be associated with an account.\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tobltgaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a bltg address.\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const string& name_, keys) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid BLTG address: ")+name_);
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZbltgSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nSpend zBLTG mints to a PIV address.\n" +
            HelpRequiringPassphrase() + "\n"
...
extern UniValue DoZbltgSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzBLTGTracker* zbltgTracker = pwalletMain->zbltgTracker.get();
    set<CMintMeta> setMints = zbltgTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CzBLTGTracker* zbltgTracker = pwalletMain->zbltgTracker.get();
    set<CMintMeta> setMints = zbltgTracker->ListMints(!fIncludeSpent, false, false);
